...
SECTIONS: Final[tuple[Section, ...]] = (
    ...
    Section("killboard.poller", "Albion gameinfo poll loop.", optional=True),
    Section("killboard.feed", "Kill/death feed routing + filtering.", optional=True),
    Section("killboard.cards", "Pillow kill-card rendering + icon cache.", optional=True),
    Section("killboard.rankings", "Leaderboards from the event store.", optional=True),
    Section("killboard.battles", "Large-fight summaries.", optional=True),
...
KEYS: Final[tuple[Key, ...]] = (
    ...
    Key(
        ...
        "bool",
        Reload.RESTART,
        "Master switch for the Albion killboard module. Also needs a guild and "
        "a feed channel; the module stays dark until all are set.",
        default=False,
    ),
    Key(
        "killboard.region",
        "str",
        Reload.RESTART,
        "Which Albion server the guild lives on — selects the API host "
        "(killboard GDD §2.2). Hitting the wrong region returns empty data.",
        default="west",
        choices=("west", "europe", "east"),
    ),
    Key(
        "killboard.guild_name",
        "str",
        Reload.RESTART,
        "Guild name, resolved to an id once at startup via /search. Leave empty "
        "if setting killboard.guild_id directly.",
        default="",
    ),
    Key(
        "killboard.guild_id",
        "str",
        Reload.RESTART,
        "Albion guild id, set directly to skip name resolution. Empty = resolve "
        "from killboard.guild_name.",
        default="",
    ),
    Key(
        "killboard.poller.interval_seconds",
        "int",
        Reload.HOT,
        "Seconds between gameinfo polls. Gentle by design (killboard GDD §15).",
        default=45,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.poller.request_timeout_seconds",
        "int",
        Reload.HOT,
        "Per-request HTTP timeout against the flaky gameinfo API.",
        default=10,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.poller.max_retries",
        "int",
        Reload.HOT,
        "Retries per poll before giving up until the next tick.",
        default=3,
        minimum=0,
    ),
    Key(
        "killboard.poller.backoff_base_seconds",
        "float",
        Reload.HOT,
        "Exponential backoff base on request failure (5, 10, 20 …).",
        default=5.0,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.poller.page_limit",
        "int",
        Reload.HOT,
        "Events per request; 51 is the endpoint maximum (killboard GDD §5.2).",
        default=51,
        minimum=1,
        maximum=51,
    ),
    Key(
        "killboard.poller.max_backfill_pages",
        "int",
        Reload.RESTART,
        "First-run backfill depth in pages (≈ the server offset ceiling). Bounds "
        "how much recent history the seed captures (killboard GDD §5.3).",
        default=20,
        minimum=0,
    ),
    Key(
    ...
    Key(
        ...
        "int",
        Reload.HOT,
        "Channel id for guild kills. 0 = unset.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.deaths_channel",
        "int",
        Reload.HOT,
        "Channel id for guild deaths (may equal kills_channel). 0 = unset.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.min_fame",
        "int",
        Reload.HOT,
        "Suppress kills below this fame from the main feed. 0 = show all.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.juicy_channel",
        "int",
        Reload.HOT,
        "Optional highlights channel for high-value kills. 0 = off.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.juicy_min_fame",
        "int",
        Reload.HOT,
        "Fame threshold for a kill to also post to juicy_channel.",
        default=2_000_000,
        minimum=0,
    ),
    Key(
    ...
    Key(
        ...
        "int",
        Reload.HOT,
        "Skip low-item-power 'naked' deaths that just clutter the feed. 0 = none.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.blob_participant_threshold",
        "int",
        Reload.HOT,
        "Kills with participant counts above this are flagged as blob/ZvZ and "
        "may route to blob_channel.",
        default=20,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.feed.blob_channel",
        "int",
        Reload.HOT,
        "Optional ZvZ channel for blob kills. 0 = keep them in the main feed.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.feed.catchup_max_posts",
        "int",
        Reload.HOT,
        "Cap on feed posts per catch-up cycle after downtime; the rest get a "
        "single 'posted N older events' summary (killboard GDD §7.3).",
        default=20,
        minimum=0,
    ),
    Key(
        "killboard.feed.post_delay_ms",
        "int",
        Reload.HOT,
        "Spacing between feed messages so a backfill burst can't hit rate limits.",
        default=750,
        minimum=0,
    ),
    Key(
        "killboard.cards.enabled",
        "bool",
        Reload.HOT,
        "Render composited kill-card images (Pillow). false = embed only.",
        default=True,
    ),
    Key(
        "killboard.cards.icon_cache_dir",
        "str",
        Reload.RESTART,
    ...
    Key(
        "killboard.cards.brand_name",
        "str",
        Reload.HOT,
        "Footer tagline on cards (the hosting Discord's name).",
        default="Dead Gaming",
    ),
    Key(
        "killboard.cards.brand_logo_path",
        "str",
        Reload.HOT,
        "PNG watermark logo path; empty = the bundled Dead roundel.",
        default="",
    ),
    Key(
        "killboard.cards.accent_color",
        "str",
        Reload.HOT,
        "Card accent colour as #RRGGBB (headers, rank boards).",
        default="#E11212",
    ),
    Key(
        "killboard.cards.show_loot_value",
        "bool",
        Reload.HOT,
        "Print estimated loot value on kill cards (needs killboard.market.enabled).",
        default=True,
    ),
    Key(
        "killboard.cards.daily_ranking_card",
        "bool",
        Reload.HOT,
        "Attach a branded Daily Ranking image to the daily scheduled post.",
        default=True,
    ),
    Key(
    ...
    Key(
        ...
        "str",
        Reload.HOT,
        "Timezone for daily/weekly/monthly ranking windows and schedules.",
        default="UTC",
    ),
    Key(
        "killboard.battles.channel",
        "int",
        Reload.HOT,
        "Optional battle-summary channel. 0 = battle posting off.",
        default=0,
        minimum=0,
    ),
    Key(
        "killboard.battles.min_players",
        "int",
        Reload.HOT,
        "Minimum guild participants for a battle to post (killboard GDD §9).",
        default=20,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.battles.min_fame",
        "int",
        Reload.HOT,
        "Minimum total fame swung for a battle to post.",
        default=5_000_000,
        minimum=0,
    ),
    Key(
        "killboard.storage.db_path",
        "str",
        Reload.RESTART,
    ...
    Key(
        ...
        "int",
        Reload.HOT,
        "No successful poll for this long → surface it in /killboard status.",
        default=30,
        minimum=0,
        exclusive_minimum=True,
    ),
    Key(
        "killboard.staleness.no_events_notice_hours",
        "int",
        Reload.HOT,
        "No NEW events for this long → note 'guild is quiet' (not an error).",
        default=6,
        minimum=0,
        exclusive_minimum=True,
    ),
