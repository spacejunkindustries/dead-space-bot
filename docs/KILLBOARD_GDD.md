...
- **Two volume/politeness guards.** `max_posts_per_scan` caps how many post (ranked by value — the biggest first), and `max_priced_per_scan` bounds the **total** number of AODP lookups a scan may make (with bounded concurrency), so the shared, rate-limited market API is never hit with the whole firehose sequentially. That budget covers *both* the sub-fame qualification prices (a sub-fame event must be priced to know whether its loot clears the bar, so these consume the budget first) *and* the fame-hit card decoration — but fame-hits are ranked on their **free** fame first and priced only for the `≤ max_posts_per_scan` survivors, after ranking, so an AODP call is never spent on a fame-hit the post cap will drop.
