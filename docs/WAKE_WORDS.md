...
openWakeWord loading one `.onnx` model — any model listed here is a drop-in:
...
then `systemctl restart cortana-brain` (wake model changes are restart-bound —
the reload receipt will say so if you try a reload instead).
